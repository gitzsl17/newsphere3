...
function($window, $document, taDOM){
	...
	var api = {
		getSelection: function(){
			var range = rangy.getSelection().getRangeAt(0);//获得用户选择区域,通过Selection对象创建Range对象。
			var container = range.commonAncestorContainer;//范围的开始点和结束点的（即它们的祖先节点）、嵌套最深的 Document 节点。
			var selection = {
				start: brException(range.startContainer, range.startOffset),
				end: brException(range.endContainer, range.endOffset),
				collapsed: range.collapsed //如果范围的开始点和结束点在文档的同一位置，则为 true，即范围是空的，或折叠的。
			};
